...
-- Sets NULL passenger_yield to rounded average for low-cost carriers
WITH avg_low_cost AS (
SELECT
...
UPDATE airline
SET passenger_yield = avg_low_cost.avg_passenger_yield
FROM avg_low_cost
WHERE passenger_yield IS NULL
AND low_cost_carrier = 'Y'

-- Sets NULL passenger_yield to rounded average for NOT low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(passenger_yield),3) AS avg_passenger_yield
FROM airline
...
-- Sets NULL ask to rounded average for low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(ask)) AS avg_ask
FROM airline
WHERE low_cost_carrier = 'Y'
)
UPDATE airline
SET ask = avg_low_cost.avg_ask
FROM avg_low_cost
WHERE ask IS NULL
AND low_cost_carrier = 'Y'

-- Sets NULL ask to rounded average for NOT low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(ask)) AS avg_ask
FROM airline
...
-- Sets NULL avg_fleet_age to rounded average for low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(avg_fleet_age),1) AS avg_avg_fleet_age
FROM airline
WHERE low_cost_carrier = 'Y'
)
UPDATE airline
SET avg_fleet_age = avg_low_cost.avg_avg_fleet_age
FROM avg_low_cost
WHERE avg_fleet_age IS NULL
AND low_cost_carrier = 'Y'

-- Sets NULL avg_fleet_age to rounded average for NOT low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(avg_fleet_age),1) AS avg_avg_fleet_age
FROM airline
...
-- Sets NULL fleet_size to rounded average for low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(fleet_size)) AS avg_fleet_size
FROM airline
WHERE low_cost_carrier = 'Y'
)
UPDATE airline
SET fleet_size = avg_low_cost.avg_fleet_size
FROM avg_low_cost
WHERE fleet_size IS NULL
AND low_cost_carrier = 'Y'

-- Sets NULL fleet_size to rounded average for NOT low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(fleet_size)) AS avg_fleet_size
FROM airline
...
-- Sets NULL aircraft_utilisation to rounded average for low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(aircraft_utilisation),1) AS avg_aircraft_utilisation
FROM airline
WHERE low_cost_carrier = 'Y'
)
UPDATE airline
SET aircraft_utilisation = avg_low_cost.avg_aircraft_utilisation
FROM avg_low_cost
WHERE aircraft_utilisation IS NULL
AND low_cost_carrier = 'Y'

-- Sets NULL aircraft_utilisation to rounded average for NOT low-cost carriers
WITH avg_low_cost AS (
SELECT
	ROUND(AVG(aircraft_utilisation),1) AS avg_aircraft_utilisation
FROM airline
...
-- Сheck if there are still missing values
SELECT 
    SUM(CASE WHEN iata_code IS NULL THEN 1 ELSE 0 END) AS null_iata_code,
    SUM(CASE WHEN airline_name IS NULL THEN 1 ELSE 0 END) AS null_airline_name,
    SUM(CASE WHEN region IS NULL THEN 1 ELSE 0 END) AS null_region,
...
--There are no missing values, all data is correct, the research and cleaning process is complete
